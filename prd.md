...
系统采用渐进式披露：先提供精简的任务信息、可发现的线索和查询能力，信任 AI 自主组织上下文，不一次性注入大量背景资料。

系统负责：

- 清晰呈现当前 Task 已记录的目标和关键约束；精简信息不应省略影响执行的要求。
- 提供可发现的关系线索，例如 `derived_from` 的任务编号与标题。
- 提供明确、可用的查询入口，让 AI 能够按需获取任务详情与 Activity / Comment。

AI 根据当前任务自主判断是否需要补充信息，以及查询哪些内容、探索到什么深度。
...
当前信息足够时，AI 可以直接执行；需要了解导出需求的背景或约束时，可以查询来源 Task：

```sh
tasknest show 42
```

不强制读取直接来源 Task，也不要求遍历整个来源链。

自主探索不保证 AI 总能识别所有信息缺口。应通过实际任务的完成质量和关键约束的满足情况检验效果；出现反复遗漏时，针对具体问题改进任务描述、关系线索或 Skill 指引。

原则：

**系统提供完成任务所需的起点、线索和查询能力，信任 AI 自主组织上下文，通过实际结果检验效果。**
...
13. 自主判断当前信息是否足够，需要补充背景或约束时，通过 CLI 按需读取 derived_from Task 及其 Activity / Comment。
14. 自主决定探索深度，不强制读取直接来源 Task，不无条件遍历整个来源链。
...
当前 Task 的信息与 `derived_from` 来源线索清晰可见，AI 可以通过 CLI 按需查询来源 Task 及其 Activity / Comment；系统不自动递归注入来源链的全部内容。

遵循 §4.4，以实际任务完成质量和关键约束的满足情况检验效果，不以查询次数或来源链遍历深度作为完成标准。
